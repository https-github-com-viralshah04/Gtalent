...
class WebsiteJobDescription(http.Controller):
    ...
    def create_applicant_summary_detail(self, **post):
        vals = {}
        if post.get('summary_detail'):
            vals['summary_detail'] = (post.get('summary_detail'))
        if vals:
    ...
    @http.route('/view_applicant_employment', type='http', auth='user', website=True, csrf=False)
    def view_applicant_employment(self, **post):
        vals = {}
        if post.get('company_name'):
            vals['company_name'] = int(post.get('company_name'))
        if post.get('designation_type_id'):
