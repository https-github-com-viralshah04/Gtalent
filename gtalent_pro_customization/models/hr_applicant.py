...
#     Adding custom fields
    ...
    recruitment_industry_id = fields.Many2one(
        'hr.recruitment.industry', 'Industry')
    ...
    educational_type_id = fields.One2many(
        'applicant.college.list', 'applicant_id', string='Educational Details')
    ...
    employment_type_id = fields.Many2many(
        'applicant.employment', string='Employment Details')
    hobbies_id = fields.One2many(
        'hr.recruitment.hobbies', 'applicant_id', string='Hobbies')
    shared_applicant_info_ids = fields.One2many(
        'share.applicant.data', 'applicant_id')
    ...
    # Relational fields
    assesment_type_id = fields.Many2many(
        'hr.recruitment.assessment', string="Assessment")
    sequence = fields.Integer(
        index=True, help="Gives the sequence order when displaying a list of bank statement lines.", default=1)
    ...
    education_count = fields.Integer(
        '# Education', compute='_compute_degree_count')
    experience_count = fields.Integer(
        '# Experience', compute='_compute_experience_count')
    skills_count = fields.Integer('# Skills', compute='_compute_skills_count')
    assessment_count = fields.Integer(
        '# Assessment', compute='_compute_assessment_count')
    projects_count = fields.Integer(
        '# Projects', compute='_compute_project_count')
    total_count = fields.Integer(
        "#Total", compute='_compute_course_cert_awards_count')
    course_count = fields.Integer(
        "#Courses", compute='_compute_course_cert_awards_count')
    certification_count = fields.Integer(
        '# Certification', compute='_compute_certificate_count')
    ...
    course_id = fields.One2many(
        'applicant.courses', 'applicant_id', string='Course Details')
    certificate_id = fields.One2many(
        'applicant.certificate', 'applicant_id', string='Certificate Details')
    awards_id = fields.One2many(
        'applicant.awards', 'applicant_id', string='Award Details')

    # Skill Related Fields
    applicant_skill_ids = fields.One2many(
        'hr.employee.skill', 'applicant_id', string="Skills")
    ...
    def default_get(self, fields):
        ...
        _logger.info(
            "CONTEXT  of HR APPLICANT IS -!!!!!!!!!!!!!!!!!!!!!!--> {}".format(self._context))
        return res
    ...
    def _compute_assessment_count(self):
        ...
        for rec in self:
            ...
            if rec.assesment_type_id:
                _logger.info(
                    "Assessment Length is ---> {}".format(len(rec.assesment_type_id)))
    ...
    def _compute_degree_count(self):
        ...
        for rec in self:
            ...
            if rec.educational_type_id:
                _logger.info(
                    "Degree Length is ---> {}".format(len(rec.educational_type_id)))
    ...
    def _compute_project_count(self):
        ...
        for rec in self:
            ...
            if rec.project_id:
                _logger.info(
                    "Project Length is ---> {}".format(len(rec.project_id)))
    ...
    def _compute_course_cert_awards_count(self):
        ...
        '''
        _logger.info(
            "Attempting to fetch Course,Certificates and Awards Count")
        count_all = []
        for rec in self:
            if rec.course_id:
    ...
    def _compute_certificate_count(self):
        ...
        for rec in self:
            ...
            if rec.certificate_id:
                _logger.info(
                    "Certificate Length is ---> {}".format(len(rec.certificate_id)))
    ...
    def _compute_course_count(self):
        ...
        for rec in self:
            ...
            if rec.course_id:
                _logger.info(
                    "Course Length is ---> {}".format(len(rec.course_id)))
    ...
    def _compute_awards_count(self):
        ...
        for rec in self:
            ...
            if rec.awards_id:
                _logger.info(
                    "Award Length is ---> {}".format(len(rec.awards_id)))
    ...
    def _compute_experience_count(self):
        ...
        for rec in self:
            ...
            if rec.employment_type_id:
                _logger.info(
                    "Experience Length is ---> {}".format(len(rec.employment_type_id)))
    ...
    def _compute_skills_count(self):
        ...
        for rec in self:
            ...
            if rec.applicant_skill_ids:
                _logger.info(
                    "Applicant Skill ID's ---> {}".format(len(rec.applicant_skill_ids)))
